...
class _ElrcCoursePageState extends State<ElrcCoursePage> {
  ...
  void _sessionChanged() {
    ...
    _generation = widget.client.session.generation;
    if (widget.client.session.isConnected) return;
    _request++;
    setState(() {
      _loading = false;
      _outline = null;
      _error = const ElrcException(ElrcErrorKind.staleSession);
    });
  }
  ...
  Future<void> _load({bool refresh = false}) async {
    ...
    try {
      ...
      final outline = await widget.client.getOutline(widget.course, refresh: refresh);
      ...
      if (mounted && request == _request) {
        setState(() {
          if (!refresh) _outline = null;
          _error = const ElrcException(ElrcErrorKind.network);
        });
      }
    } finally {
