...
class _GenericWebViewPageState extends State<GenericWebViewPage> {
  ...
  bool get _isOpen => mounted && !_invalidated && !_closing;
    ...
    await _controller.setNavigationDelegate(
      NavigationDelegate(
        onNavigationRequest: (request) {
          if (_closing && request.url == 'about:blank') {
            return NavigationDecision.navigate;
          }
          if (!_isOpen) return NavigationDecision.prevent;
          final suffixes = widget.allowedHostSuffixes;
          if (suffixes == null || isAllowedWebViewNavigation(request.url, suffixes)) {
            return NavigationDecision.navigate;
          }
          final uri = Uri.tryParse(request.url);
          if (!request.isMainFrame) {
            return uri?.scheme == 'https' && uri!.userInfo.isEmpty
                ? NavigationDecision.navigate
                : NavigationDecision.prevent;
          }
          _showBlockedNavigation(uri);
  ...
  void _showBlockedNavigation(Uri? uri) {
    final canOpen = uri != null &&
        uri.userInfo.isEmpty &&
        ((uri.scheme == 'https' && uri.host.isNotEmpty) ||
            ({'tel', 'mailto'}.contains(uri.scheme) && uri.path.isNotEmpty));
    final message = !canOpen
        ? '无法打开此链接。'
        : uri.scheme == 'https'
            ? '此链接将前往 ${uri.host}。'
            : uri.scheme == 'tel'
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        ...
        action: canOpen
            ? SnackBarAction(
                label: '外部打开',
                onPressed: () => unawaited(_openExternal(uri)),
              )
            : null,
      ),
    );
  ...
  Future<void> _openExternal(Uri uri) async {
    ...
    } catch (_) {/* No installed app may support the link. */}
