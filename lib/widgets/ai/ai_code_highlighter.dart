...
/// Syntax-highlights AI code blocks using the `highlight` package, mapping
/// token classes to a VS Code–dark-ish palette. Returns `null` (plain
/// monospace) when the language is unknown or parsing fails, so rendering
/// never breaks.
///
/// This matches the `CodeHighlighter` typedef from flutter_ai_elements
/// structurally (so it can be passed to `AiResponse.codeHighlighter`).
/// Ported verbatim from the flutter_ai demo (`demo/lib/code_highlighter.dart`)
/// so TechPie's code blocks match the demo's look exactly.
List<TextSpan>? techpieCodeHighlighter(
  String code,
  String? language,
  TextStyle base,
) {
