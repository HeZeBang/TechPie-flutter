...
class ElrcClient {
  ...
  final _remoteOutlines = <String, ElrcRecordingOutline>{};
  final _outlineRequests = <String, int>{};
  int _generation = 0;
  ...
  void _sessionChanged() {
    ...
    _generation = session.generation;
    _remoteOutlines.clear();
    _outlineRequests.clear();
  ...
  Future<ElrcRecordingOutline> getOutline(ElrcCourse course, {bool refresh = false}) async {
    if (_disposed) throw const ElrcException(ElrcErrorKind.staleSession);
    final generation = session.generation;
    session.check(generation);
    if (refresh) {
      final request = (_outlineRequests[course.key] ?? 0) + 1;
      _outlineRequests[course.key] = request;
      final outline = await _getRemoteOutline(course);
      session.check(generation);
      if (_disposed) throw const ElrcException(ElrcErrorKind.staleSession);
      if (_outlineRequests[course.key] == request) _remoteOutlines[course.key] = outline;
      return outline;
    }
    final current = _remoteOutlines[course.key];
    if (current != null) return current;
    final snapshot = await _snapshot(course);
    final refreshed = _remoteOutlines[course.key];
  ...
  Future<ElrcRecordingSources> getSources(
    ...
    try {
      final snapshot = _remoteOutlines.containsKey(course.key) ? null : await _snapshot(course);
      if (snapshot != null && !_remoteOutlines.containsKey(course.key)) {
